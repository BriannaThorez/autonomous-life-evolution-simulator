...
const SensoryDropdown: React.FC<Props> = ({
    ...
    const Toggle = ({ active, onClick, label, icon }: { active: boolean, onClick: () => void, label: string, icon: string }) => (
        <button
            onClick={onClick}
            className={`w-full flex items-center justify-between rounded transition-all juice-interactive
        ...
        >
            <span className="text-[0.56rem] font-black uppercase tracking-[0.16em] text-[#E5EFC1] opacity-80 flex items-center gap-2">
    ...
    return (
        <Tooltip title="Sensory Overlays" content="Toggle global visibility of sensory radiuses. When OFF, senses only appear dynamically when an organism actively uses them." position="bottom">
            ...
            >
                ...
                <button
                    className="glass-modular rounded flex items-center justify-center transition-all h-full"
                    style={{
                ...
                {isOpen && (
                    <div
                        className="absolute top-full right-0 glass-modular rounded-md flex flex-col gap-2 animate-in slide-in-from-top-2 duration-200"
                        style={{
                            zIndex: 1001,
                    ...
                    >
                        <div className="flex items-center justify-between border-b border-[#39AEA9]/20" style={{ paddingBottom: '0.32rem', marginBottom: '0.14rem' }}>
                            <span className="text-[0.58rem] litho-text font-black uppercase tracking-[0.16em] text-[#39AEA9]">Global Override</span>
